...
describe('Splat Pipeline Integration', () => {
  it('full pipeline: blendshapes to FLAME to Gaussian positions', () => {
    ...
    // 2 Gaussians, one on each face, both at local origin
    const updater = new GaussianUpdater(
      flameAssets.faces,
      new Uint32Array([0, 1]),
      new Float32Array([0,0,0, 0,0,0]),     // local xyz
      new Float32Array([0,0,0, 0,0,0]),     // local log scale
      new Float32Array([1,0,0,0, 1,0,0,0]), // identity rot WXYZ
    )
    ...
    // Neutral pose
    const neutralFlame = mapper.convert({})
    const neutralVerts = flameModel.deform(shape, neutralFlame)
    updater.updateFaceProperties(neutralVerts)
    updater.transformGaussians(positions, logScales, rotations)
    const neutralX = positions[0]

    // With expression
    const exprFlame = mapper.convert({ eyeBlinkLeft: 1.0 } as BlendshapeMap)
    const exprVerts = flameModel.deform(shape, exprFlame)
    updater.updateFaceProperties(exprVerts)
    updater.transformGaussians(positions, logScales, rotations)
    const exprX = positions[0]

    // Expression changes vertex 0 x by 0.1, which changes face centroid
